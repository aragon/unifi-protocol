...
import {ISingleStrategyManager} from "./interfaces/ISingleStrategyManager.sol";
 ...
 */
contract SingleStrategyManager is ISingleStrategyManager, DaoAuthorizable {
    VaultaireVault public vault; // The vault that owns this manager
    ...
    uint256 public investmentRatio = 8000; // Default 80%

    // Reserved assets ratio (in basis points)
    // Minimum amount of assets that should remain in the vault
    uint256 public reservedRatio = 2000; // Default 20%

    event InvestmentRatioUpdated(uint256 newRatio);
    event ReservedRatioUpdated(uint256 newRatio);

    /**
     * @dev Error thrown when a non-vault address attempts to call a vault-only function.
     */
    error OnlyVaultAllowed(address caller, VaultaireVault vault);

    /**
     * @dev Error thrown when a non-vault and non-DAO address attempts to call a restricted function.
     */
    error OnlyVaultOrDAOAllowed(address caller, VaultaireVault vault, address dao);
    ...
    /**
     * @dev Error thrown when combined investment and reserved ratios exceed 100%.
     */
    error CombinedRatiosExceed100Percent(uint256 investmentRatio, uint256 reservedRatio);

    modifier onlyVault() {
        if (msg.sender != address(vault)) {
            revert OnlyVaultAllowed(msg.sender, vault);
        }
        _;
    }

    constructor(VaultaireVault vault_, IDAO dao_) DaoAuthorizable(dao_) {
        vault = vault_;
    }
     ...
     */
    function setStrategy(IVaultAllocationStrategy strategy) external override {
        if (msg.sender != address(vault) && msg.sender != address(dao())) {
            revert OnlyVaultOrDAOAllowed(msg.sender, vault, address(dao()));
        }
        _strategy = strategy;
     ...
     * @dev Can only be called by the vault or the DAO
     */
    function clearStrategy() external override {
        if (msg.sender != address(vault) && msg.sender != address(dao())) {
            revert OnlyVaultOrDAOAllowed(msg.sender, vault, address(dao()));
        }
     ...
     */
    function getStrategy() external view override returns (IVaultAllocationStrategy strategy) {
        return _strategy;
    }
     ...
     */
    function allocate(uint256 assets) external override onlyVault returns (uint256 totalAllocated) {
     ...
     */
    function deallocate(uint256 assets) external override onlyVault returns (uint256 totalDeallocated) {
        if (address(_strategy) == address(0)) return 0;
     ...
     */
    function rebalance() internal returns (uint256 assetsAllocated, uint256 assetsDeallocated) {
        ...
        if (strategyBalance < targetStrategyAmount) {
            ...
            uint256 minReserve = (totalAssets * reservedRatio) / 10000;
    ...
    function setInvestmentRatio(uint256 newRatio) external {
        ...
        // Ensure investmentRatio + reservedRatio <= 10000 (100%)
        if (newRatio + reservedRatio > 10000) {
            revert CombinedRatiosExceed100Percent(newRatio, reservedRatio);
        }
    ...
    /**
     * @notice Sets the reserved assets ratio (minimum kept in vault).
     * @dev Only the DAO can call this function
     * @param newRatio The new reserved ratio in basis points (e.g., 2000 = 20%)
     */
    function setReservedRatio(uint256 newRatio) external {
        if (msg.sender != address(dao())) {
            revert OnlyDAOAllowed(msg.sender, address(dao()));
        }
        ...
        // Ensure investmentRatio + reservedRatio <= 10000 (100%)
        if (investmentRatio + newRatio > 10000) {
            revert CombinedRatiosExceed100Percent(investmentRatio, newRatio);
        }

        reservedRatio = newRatio;
        emit ReservedRatioUpdated(newRatio);
    }
}
