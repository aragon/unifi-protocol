...
contract VaultaireVault4626TokenTest is BaseVaultaire4626TokenTest {
    ...
    function test_StrategyCalculatesTheYield() external {
        ...
        assertEq(vault.calculateYield(), 0, "Yield should be zero initially");

        // Setup: deposit the yield (fake)
        uint256 yieldAmount = 10_000 ether;
        assetCollateral.transfer(address(asset), yieldAmount);

        assertEq(vault.calculateYield(), (yieldAmount / 2) - 1, "Yield should be equal to the deposited amount");
        ...
        assertEq(vault.calculateYield(), yieldAmount - 1, "Yield should be equal to the deposited amount");
    }
    ...
    function test_StrategyHarvestsTheYield() external {
        ...
        vault.harvest();
        assertEq(vault.calculateYield(), 0, "Yield should've been harvested");
        assertEq(asset.balanceOf(address(dao)), yieldAmount - 1, "DAO should've received the yield");
    }
}
